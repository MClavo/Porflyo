...
class ProviderRepoLambdaHandlerTest {
    ...
    @Test
    void should_returnInternalServerError_when_jwtPortThrowsException() {
        // given
        String sessionCookie = "valid-session-token";
        APIGatewayV2HTTPEvent event = createEventWithCookie("session", sessionCookie);
        
        given(authUseCase.extractClaims(sessionCookie)).willThrow(new RuntimeException("JWT parsing error"));
        ...
        assertThat(response.getBody()).isEqualTo("{\"error\": \"JWT parsing error\"}");
    }

    @Test
    void should_returnInternalServerError_when_providerUseCaseThrowsException() {
        // given
        String sessionCookie = "valid-session-token";
        String userId = "user-123";
        UserClaims userClaims = new UserClaims(userId, Instant.now(), Instant.now().plusSeconds(3600));
        ...
        given(authUseCase.extractClaims(sessionCookie)).willReturn(userClaims);
        given(providerUseCase.getUserRepos(new UserId(userId)))
                .willThrow(new RuntimeException("Provider service error"));
        ...
        // then
        assertThat(response.getStatusCode()).isEqualTo(500);
        assertThat(response.getBody()).isEqualTo("{\"error\": \"Provider service error\"}");
    }
    ...
    void should_returnInternalServerError_when_jsonMapperThrowsException() throws Exception {
        ...
        ProviderRepo repo = new ProviderRepo("repo1", "Test Repo 1", "https://github.com/user/repo1");
        ...
        given(authUseCase.extractClaims(sessionCookie)).willReturn(userClaims);
        given(providerUseCase.getUserRepos(new UserId(userId))).willReturn(repos);
        given(jsonMapper.writeValueAsString(repos)).willThrow(new RuntimeException("JSON serialization error"));
        ...
        // then
        assertThat(response.getStatusCode()).isEqualTo(500);
        assertThat(response.getBody()).isEqualTo("{\"error\": \"JSON serialization error\"}");
    }
